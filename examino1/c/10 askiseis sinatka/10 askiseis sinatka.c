...
int main(void){
	...
	// Tis 2 askiseis pou perilamvane h askhsh 1 tis evala mazi
	...
	int ergasia6(void){
		...
		int checkyear(year, month){
			if (year%4 != 0 && year%400 != 0) return daypermonth1[month]; else return daypermonth2[month];
		}
		if (d1 > checkyear(y1, m1-1) || m1 > 12) {printf("H hmeromhnia genisis einai lathos!"); return 0;}
		if (d2 > checkyear(y2, m2-1) || m2 > 12) {printf("H torinh hmeromhnia einai lathos!"); return 0;}
		if (y1 > y2 || (y1 == y2 && m1 > m2) || (y1 == y2 && m1 == m2 && d1 >= d2)) {printf("H hmeromhnia genisis proigeitai ths torinis hmeromhnias!"); return 0;}
	...
	int ergasia8(void){
		...
		for (i=0; done == 0; i++) {
			...
			else if (mop == '/') {if (num1 != 0 && num2 == 0) {printf("Den mporeis na diaireseis me to 0!"); return 0;} else {printf("%f / %f = %f", num1, num2, num1 / num2); done = 1;}}
		}
	}
	...
	int ergasia9(void){
		...
		for (i=1; i<number; i+=2) {
			if (number%i == 0 && i != 1 && i != number) {printf("O arithmos %i den einai protos", number); return 0;}
		}
