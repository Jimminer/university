...
// void deleteGraph(Graph *graph);("%i\n", temp->key);
//             temp = temp->RChild;
//             printf("%i\n", temp->key);
//             started = 1;
//             continue;
...
// GRData removeGraph(Graph *graph, int index);
...
// void internal_findPathGraph(Graph *graph, GRVert *grvert, int index, LList *llist){
//     int i, visited = 0;

//     GREdge *gredge = grvert->nextEdge;

//     LLData data;

//     if (!internal_isVisitedInPathGraph(llist, gredge->connectedVert->index)){
//         data.index = grvert->index;
//         addLList(llist, 0, data);
//         printf("Added %i\n", grvert->index);
//     }

//     if (gredge == NULL) return;

//     printf("%i -> ", grvert->index);

//     while (gredge->index != index){
//         gredge = gredge->nextEdge;
//         if (gredge == NULL) return;
//         if (internal_isVisitedInPathGraph(llist, gredge->connectedVert->index)) continue;
//         internal_findPathGraph(graph, gredge->connectedVert, index, llist);
//     }
//     printf("\n");
