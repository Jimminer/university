...
void changepin(int, unsigned short);
...
int main(){
    ...
    for(;;){
        ...
        switch(mainpage()){
            case 1: {enimerosi(); getch_(); break;}
            case 2: {metafora(); getch_(); break;}
            case 3: {analipsi(); getch_(); break;}
            case 4: {katathesi(); getch_(); break;}
            case 5: {pliromes(); getch_(); break;}
            case 6: {allagipin(); getch_(); break;}
            case 101: {ipalilos1(); getch_(); break;}
            case 102: {ipalilos2(); getch_(); break;}
...
void changepin(int account, unsigned short pin){
    FILE *file;
    accdata temp;
    if ((file = fopen("accounts", "rb+")) != NULL){
        while (fread(&temp.account, sizeof(temp.account), 1, file) > 0){
