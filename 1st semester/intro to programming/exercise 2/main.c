...
void ergasia4(){
	...
	scanf("%20s", arr);
	for(i=strlen(arr)-1; i>=0; i--) printf("%c", arr[(int)i]);
	printf("\n");
}
...
int main(void){
	...
	for (;;) {
		...
		scanf("%i", &ergasia);
		switch(ergasia){
			case 1: {ergasia1(); getchar(); return 0;}
			case 2: {ergasia2(); getchar(); return 0;}
			case 3: {ergasia3(); getchar(); return 0;}
			case 4: {ergasia4(); getchar(); return 0;}
			default:
